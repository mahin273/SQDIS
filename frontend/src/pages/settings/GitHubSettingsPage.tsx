...
export function GitHubSettingsPage() {
  ...
  useEffect(() => {
    ...
    if (connected === 'success') {
      setOauthFeedback({
        type: 'success',
        message: 'GitHub integration connected successfully via 1-Click OAuth! Your repositories and metrics are now synchronizing.',
      })
      ...
      setOauthFeedback({
        type: 'error',
        message: `GitHub connection failed: ${decodeURIComponent(error)}`,
      })
      window.history.replaceState({}, document.title, window.location.pathname)
    }
  ...
  const handleOAuthConnect = async () => {
    ...
    setConnectError(null)
    try {
      const res = await githubService.getOAuthAuthorizeUrl()
      if (res?.url) {
        window.location.href = res.url
      }
    } catch (err: any) {
      const msg = err?.response?.data?.message || err?.message || 'Failed to initiate GitHub OAuth. Please try again.'
      setConnectError(msg)
  ...
  return (
    <div className="space-y-6">
      ...
      {oauthFeedback && (
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <p className="text-sm font-medium">{oauthFeedback.message}</p>
          </div>
          <button
      ...
      <Modal
        ...
        description="Link your GitHub account via 1-Click OAuth (Recommended) or a Personal Access Token to synchronize your repositories, track quality metrics, and configure webhook telemetry."
        size="lg"
        footer={
          <div className="flex items-center justify-between w-full">
            <Button
              ...
              disabled={connectMutation.isPending || isOAuthLoading}
            >
              Cancel
            </Button>
            <div className="flex items-center gap-2">
              <Button
                ...
                disabled={!patInput.trim() || isValidating || connectMutation.isPending || isOAuthLoading}
                className="gap-1.5"
              >
              ...
              <Button
                ...
                disabled={!patInput.trim() || isOAuthLoading}
                className="gap-2 bg-blue-600 hover:bg-blue-700 text-white"
              >
                <Link2 className="h-4 w-4" /> Connect with PAT
              </Button>
            </div>
          </div>
        }
      >
        <form onSubmit={handleConnectSubmit} className="space-y-5">
          ...
          <div className="p-4 bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 text-white rounded-lg border border-slate-700 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                <Github className="h-5 w-5 text-white" />
                <span className="text-sm font-semibold text-white">1-Click GitHub OAuth</span>
                <span className="text-[10px] uppercase font-bold tracking-wider bg-blue-500/30 text-blue-300 border border-blue-400/30 px-1.5 py-0.5 rounded">
                  Recommended
                </span>
              </div>
              <p className="text-xs text-slate-300">
                Authorize directly via GitHub's secure OAuth consent screen. No manual token creation or copying required.
              </p>
            </div>
            <Button
              type="button"
              ...
              className="shrink-0 bg-blue-600 hover:bg-blue-500 text-white font-medium text-xs gap-2"
            >
              {isOAuthLoading ? <RefreshCw className="h-4 w-4 animate-spin" /> : <Github className="h-4 w-4" />}
              {isOAuthLoading ? 'Redirecting to GitHub...' : 'Connect with GitHub'}
            </Button>
          </div>
